...
def index():
  ...
  if not session.get('logged_in'):
    ...
    cursor = g.conn.execute("SELECT O.picture, D.picture1, D.picture2, D.name, D.breed, D.age, D.weight, D.play_intensity, O.phone, O.email FROM owner O LEFT JOIN dog_owned_by DOB on O.owner_id=DOB.owner_id LEFT JOIN dog D on DOB.dog_id=D.dog_id LIMIT 10 ")
    names = []
